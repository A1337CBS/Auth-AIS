...
using namespace C1665;

/* test for P=O point-at-infinity */
int C1665::ECP_isinf(ECP *P)
{

#if CURVETYPE_C1665==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_C1665==WEIERSTRASS
...
static void ECP_cswap(C1665::ECP *P, C1665::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_C1665!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(C1665::ECP *P, C1665::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_C1665!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_C1665!=MONTGOMERY

#if CURVETYPE_C1665!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(C1665::ECP *P, C1665::ECP W[], sign32 b)
{
    C1665::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int C1665::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void C1665::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_C1665!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=16 */
int C1665::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int C1665::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int C1665::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
#if CURVETYPE_C1665==EDWARDS || CURVETYPE_C1665==WEIERSTRASS
...
/* SU=120 */
void C1665::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void C1665::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void C1665::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_C1665==MONTGOMERY
    BIG x;
    ECP_get(x, P);
    W->len = MODBYTES_B168_60; // + 1;
    //W->val[0] = 0x06;
    ...
    {
        W->val[0] = 0x04;
        W->len = 2 * MODBYTES_B168_60 + 1;
        BIG_toBytes(&(W->val[1]), x);
        BIG_toBytes(&(W->val[MODBYTES_B168_60 + 1]), y);
    }
#endif
}
...
int C1665::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_C1665==MONTGOMERY
    BIG x;
    ...
    if (typ == 0x04)
    {
        BIG_fromBytes(y, &(W->val[MODBYTES_B168_60 + 1]));
...
void C1665::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void C1665::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_C1665==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  C1665::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void C1665::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void C1665::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_C1665==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void C1665::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B168_60 * BASEBITS_B168_60 + 1) / 2];
    int i, a, b, s, ns, nb;
...
void C1665::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_C1665==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_C1665 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_C1665 > 2)
        FP_imul(&t,&t,QNRI_C1665); // precomputed QNR
...
void C1665::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
