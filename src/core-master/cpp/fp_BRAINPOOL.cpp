...
/* So when multiplying two numbers, their product *must* be less than MODBITS_BRAINPOOL+BASEBITS_B256_56*NLEN_B256_56 */
/* Results *may* be one bit bigger than MODBITS_BRAINPOOL */

#if MODTYPE_BRAINPOOL == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void BRAINPOOL::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void BRAINPOOL::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void BRAINPOOL::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_BRAINPOOL);
    ...
    if (MConst < NEXCESS_B256_56)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B256_56 - 1];
        r[NLEN_B256_56 - 1] &= TMASK_BRAINPOOL;
        r[0] += MConst * ((tw >> TBITS_BRAINPOOL));
    }
    else
    {
        ...
        r[NLEN_B256_56 - 1] &= TMASK_BRAINPOOL;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_BRAINPOOL) + (v << (BASEBITS_B256_56 - TBITS_BRAINPOOL))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_BRAINPOOL) + (v << (BASEBITS_B256_56 - TBITS_BRAINPOOL)));
#endif
    }
    BIG_norm(r);
}
...
void BRAINPOOL::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_BRAINPOOL);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_BRAINPOOL / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_BRAINPOOL / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B256_56 - 1] >> TBITS_BRAINPOOL;
...
/*
    ...
    BIG_split(t2, b2, d, MBITS_BRAINPOOL);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B256_56 - 1] >> TBITS_BRAINPOOL; // + (carry<<(BASEBITS_B256_56-TBITS_BRAINPOOL));
    r[NLEN_B256_56 - 1] &= TMASK_BRAINPOOL;
...
void BRAINPOOL::FP_mod(BIG a, DBIG d)
{
    int i;
...
void BRAINPOOL::FP_from_int(FP *x,int a)
{
    BIG w;
...
void BRAINPOOL::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void BRAINPOOL::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void BRAINPOOL::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void BRAINPOOL::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void BRAINPOOL::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int BRAINPOOL::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
void BRAINPOOL::FP_rawoutput(FP *r)
{
    BIG_rawoutput(r->g);
}
...
/* SU= 88 */
void BRAINPOOL::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;
...
/* SU= 136 */
void BRAINPOOL::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
/* Set r=a^2 mod m */
/* SU= 88 */
void BRAINPOOL::FP_sqr(FP *r, FP *a)
{
    DBIG d;
...
/* SU= 16 */
/* Set r=a+b */
void BRAINPOOL::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
...
/* Set r=a-b mod m */
/* SU= 56 */
void BRAINPOOL::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
/* Set r=-a mod Modulus */
/* SU= 64 */
void BRAINPOOL::FP_neg(FP *r, FP *a)
{
    int sb;
    BIG m;
...
/* Set r=a/2. */
/* SU= 56 */
void BRAINPOOL::FP_div2(FP *r, FP *a)
{
    BIG m;
    BIG w;
    BIG_rcopy(m, Modulus);
...
void BRAINPOOL::FP_pow(FP *r, FP *a, BIG b)
{
...
void BRAINPOOL::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_BRAINPOOL;
    FP xp[11], t, key;
...
void BRAINPOOL::FP_invsqrt(FP *r,FP *x)
{
#if MODTYPE_BRAINPOOL==PSEUDO_MERSENNE  || MODTYPE_BRAINPOOL==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
/* Modular inversion */
void BRAINPOOL::FP_inv(FP *r,FP *x)
{
    int i,e=PM1D2_BRAINPOOL;
    FP s,t;
    FP_norm(x);
...
void BRAINPOOL::FP_sqrt(FP *r, FP *a, FP* h)
{
