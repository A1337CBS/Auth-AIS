...
/*
    ...
    {
        k=i-(NLEN_B256_56-1);
        while (k<=NLEN_B256_56-1) {t+=(dchunk)a[k]*b[i-k]; k++;}
        c[i]=(chunk)t & BMASK_B256_56;
        t = t >> BASEBITS_B256_56;
    }
...
*/
    ...
    for (k = NLEN_B256_56; k < 2 * NLEN_B256_56 - 1; k++)
    {
        s -= d[k - NLEN_B256_56];
        t += s;
        for (i=1+k/2;i<NLEN_B256_56;i++) t += (dchunk)(a[i] - a[k - i]) * (b[k - i] - b[i]);
        c[k] = (chunk)t & BMASK_B256_56;
        t = t >> BASEBITS_B256_56;
    }
    c[2 * NLEN_B256_56 - 1] = (chunk)t;
...
#ifdef DEBUG_NORM
    if ((d[DMPV_B256_56] != 1 && d[DMPV_B256_56] != 0) || d[DMNV_B256_56] != 0) printf("Input to redc not normed\n");
#endif
...
/*
    ...
    {
        k=i-(NLEN_B256_56-1);
        while (k<=NLEN_B256_56-1) {t += (dchunk)v[k]*md[i-k]; k++;}
...
*/
    ...
    {
        t = t + s;
        for (i=1+k/2;i<NLEN_B256_56;i++) t += (dchunk)(v[k - i] - v[i]) * (md[i] - md[k - i]);
