...
/* So when multiplying two numbers, their product *must* be less than MODBITS_ANSSI+BASEBITS_B256_56*NLEN_B256_56 */
/* Results *may* be one bit bigger than MODBITS_ANSSI */

#if MODTYPE_ANSSI == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void ANSSI::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void ANSSI::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void ANSSI::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_ANSSI);
    ...
    if (MConst < NEXCESS_B256_56)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B256_56 - 1];
        r[NLEN_B256_56 - 1] &= TMASK_ANSSI;
        r[0] += MConst * ((tw >> TBITS_ANSSI));
    }
    else
    {
        ...
        r[NLEN_B256_56 - 1] &= TMASK_ANSSI;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_ANSSI) + (v << (BASEBITS_B256_56 - TBITS_ANSSI))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_ANSSI) + (v << (BASEBITS_B256_56 - TBITS_ANSSI)));
#endif
    }
    BIG_norm(r);
}
...
void ANSSI::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_ANSSI);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_ANSSI / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_ANSSI / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B256_56 - 1] >> TBITS_ANSSI;
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B256_56 - 1] >> TBITS_ANSSI; // + (carry<<(BASEBITS_B256_56-TBITS_ANSSI));
    r[NLEN_B256_56 - 1] &= TMASK_ANSSI;
...
void ANSSI::FP_mod(BIG a, DBIG d)
{
    int i;
...
void ANSSI::FP_from_int(FP *x,int a)
{
    BIG w;
...
void ANSSI::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void ANSSI::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void ANSSI::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void ANSSI::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void ANSSI::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int ANSSI::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
/* SU= 88 */
void ANSSI::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;
...
/* SU= 136 */
void ANSSI::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
/* SU= 16 */
/* Set r=a+b */
void ANSSI::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
...
/* Set r=a-b mod m */
/* SU= 56 */
void ANSSI::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
void ANSSI::FP_pow(FP *r, FP *a, BIG b)
{
...
void ANSSI::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_ANSSI;
    FP xp[11], t, key;
...
void ANSSI::FP_invsqrt(FP *r,FP *x)
{
#if MODTYPE_ANSSI==PSEUDO_MERSENNE  || MODTYPE_ANSSI==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
void ANSSI::FP_sqrt(FP *r, FP *a, FP* h)
{
