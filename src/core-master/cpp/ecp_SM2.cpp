...
int SM2::ECP_isinf(ECP *P)
{

#if CURVETYPE_SM2==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_SM2==WEIERSTRASS
    return (FP_iszilch(&(P->x)) && FP_iszilch(&(P->z)));
#endif
#if CURVETYPE_SM2==MONTGOMERY
...
static void ECP_cswap(SM2::ECP *P, SM2::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_SM2!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(SM2::ECP *P, SM2::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_SM2!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_SM2!=MONTGOMERY

#if CURVETYPE_SM2!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(SM2::ECP *P, SM2::ECP W[], sign32 b)
{
    SM2::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int SM2::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* SU=16 */
int SM2::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int SM2::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int SM2::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
#if CURVETYPE_SM2==EDWARDS || CURVETYPE_SM2==WEIERSTRASS
...
void SM2::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_SM2==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int SM2::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_SM2==MONTGOMERY
    BIG x;
...
void SM2::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
void SM2::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
void SM2::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void SM2::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_SM2==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_SM2 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_SM2 > 2)
        FP_imul(&t,&t,QNRI_SM2); // precomputed QNR
...
void SM2::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
