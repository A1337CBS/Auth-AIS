...
int JUBJUB::ECP_isinf(ECP *P)
{

#if CURVETYPE_JUBJUB==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_JUBJUB==WEIERSTRASS
    return (FP_iszilch(&(P->x)) && FP_iszilch(&(P->z)));
#endif
#if CURVETYPE_JUBJUB==MONTGOMERY
...
static void ECP_cswap(JUBJUB::ECP *P, JUBJUB::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_JUBJUB!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(JUBJUB::ECP *P, JUBJUB::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_JUBJUB!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_JUBJUB!=MONTGOMERY

#if CURVETYPE_JUBJUB!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(JUBJUB::ECP *P, JUBJUB::ECP W[], sign32 b)
{
    JUBJUB::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int JUBJUB::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void JUBJUB::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_JUBJUB!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
int JUBJUB::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int JUBJUB::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int JUBJUB::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int JUBJUB::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int JUBJUB::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
#if CURVETYPE_JUBJUB==EDWARDS || CURVETYPE_JUBJUB==WEIERSTRASS
...
/* SU=120 */
void JUBJUB::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void JUBJUB::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void JUBJUB::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_JUBJUB==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int JUBJUB::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_JUBJUB==MONTGOMERY
    BIG x;
...
void JUBJUB::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void JUBJUB::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_JUBJUB==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  JUBJUB::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void JUBJUB::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void JUBJUB::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_JUBJUB==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void JUBJUB::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void JUBJUB::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_JUBJUB==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_JUBJUB == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_JUBJUB > 2)
        FP_imul(&t,&t,QNRI_JUBJUB); // precomputed QNR
...
void JUBJUB::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int JUBJUB::ECP_generator(ECP *G)
{
    BIG x, y;
