...
/* So when multiplying two numbers, their product *must* be less than MODBITS_JUBJUB+BASEBITS_B256_56*NLEN_B256_56 */
/* Results *may* be one bit bigger than MODBITS_JUBJUB */

#if MODTYPE_JUBJUB == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void JUBJUB::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void JUBJUB::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void JUBJUB::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_JUBJUB);
    ...
    if (MConst < NEXCESS_B256_56)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B256_56 - 1];
        r[NLEN_B256_56 - 1] &= TMASK_JUBJUB;
        r[0] += MConst * ((tw >> TBITS_JUBJUB));
    }
    else
    {
        ...
        r[NLEN_B256_56 - 1] &= TMASK_JUBJUB;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_JUBJUB) + (v << (BASEBITS_B256_56 - TBITS_JUBJUB))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_JUBJUB) + (v << (BASEBITS_B256_56 - TBITS_JUBJUB)));
#endif
    }
    BIG_norm(r);
}
...
#if MODTYPE_JUBJUB == GENERALISED_MERSENNE
...
void JUBJUB::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_JUBJUB);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_JUBJUB / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_JUBJUB / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B256_56 - 1] >> TBITS_JUBJUB;
...
/*
    ...
    BIG_split(t2, b2, d, MBITS_JUBJUB);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B256_56 - 1] >> TBITS_JUBJUB; // + (carry<<(BASEBITS_B256_56-TBITS_JUBJUB));
    r[NLEN_B256_56 - 1] &= TMASK_JUBJUB;
...
#if MODTYPE_JUBJUB == MONTGOMERY_FRIENDLY
...
void JUBJUB::FP_mod(BIG a, DBIG d)
{
    int i;
...
void JUBJUB::FP_from_int(FP *x,int a)
{
    BIG w;
...
void JUBJUB::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void JUBJUB::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void JUBJUB::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void JUBJUB::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void JUBJUB::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int JUBJUB::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
/* SU= 88 */
void JUBJUB::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;

    if ((sign64)a->XES * b->XES > (sign64)FEXCESS_JUBJUB)
    {
#ifdef DEBUG_REDUCE
...
/* SU= 136 */
void JUBJUB::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
#if MODTYPE_JUBJUB==PSEUDO_MERSENNE || MODTYPE_JUBJUB==GENERALISED_MERSENNE
...
/* Set r=a^2 mod m */
/* SU= 88 */
void JUBJUB::FP_sqr(FP *r, FP *a)
{
    DBIG d;

    if ((sign64)a->XES * a->XES > (sign64)FEXCESS_JUBJUB)
    {
#ifdef DEBUG_REDUCE
        printf("Product too large - reducing it\n");
#endif
...
/* SU= 16 */
/* Set r=a+b */
void JUBJUB::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
    if (r->XES > FEXCESS_JUBJUB)
    {
#ifdef DEBUG_REDUCE
...
/* Set r=a-b mod m */
/* SU= 56 */
void JUBJUB::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
void JUBJUB::FP_pow(FP *r, FP *a, BIG b)
{
...
void JUBJUB::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_JUBJUB;
    FP xp[11], t, key;
...
// phase 1
    ...
    n = MODBITS_JUBJUB;
#endif
#if MODTYPE_JUBJUB==GENERALISED_MERSENNE  // Goldilocks ONLY
    n = MODBITS_JUBJUB / 2;
#endif
...
void JUBJUB::FP_invsqrt(FP *r,FP *x)
{
#if MODTYPE_JUBJUB==PSEUDO_MERSENNE  || MODTYPE_JUBJUB==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
int JUBJUB::FP_qr(FP *x,FP *h)
{
    FP r;
    int i,e=PM1D2_JUBJUB;
    FP_invsqrt(&r,x);
    if (h!=NULL)
        FP_copy(h,&r);
...
void JUBJUB::FP_sqrt(FP *r, FP *a, FP* h)
{
    int i,j,k,u,e=PM1D2_JUBJUB;
    FP v,g,t,b;
    BIG m;
