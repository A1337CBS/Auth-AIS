...
using namespace BRAINPOOL;

/* test for P=O point-at-infinity */
int BRAINPOOL::ECP_isinf(ECP *P)
{
...
static void ECP_cswap(BRAINPOOL::ECP *P, BRAINPOOL::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_BRAINPOOL!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(BRAINPOOL::ECP *P, BRAINPOOL::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_BRAINPOOL!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
static void ECP_select(BRAINPOOL::ECP *P, BRAINPOOL::ECP W[], sign32 b)
{
    BRAINPOOL::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int BRAINPOOL::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void BRAINPOOL::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_BRAINPOOL!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=56 */
void BRAINPOOL::ECP_rhs(FP *v, FP *x)
{
#if CURVETYPE_BRAINPOOL==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
...
int BRAINPOOL::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int BRAINPOOL::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int BRAINPOOL::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int BRAINPOOL::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int BRAINPOOL::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
/* SU=160 */
void BRAINPOOL::ECP_affine(ECP *P)
{
    FP one, iz;
...
/* SU=120 */
void BRAINPOOL::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void BRAINPOOL::ECP_output(ECP *P)
{
    BIG x, y;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void BRAINPOOL::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void BRAINPOOL::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_BRAINPOOL==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int BRAINPOOL::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_BRAINPOOL==MONTGOMERY
    BIG x;
...
void BRAINPOOL::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void BRAINPOOL::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_BRAINPOOL==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  BRAINPOOL::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void BRAINPOOL::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void BRAINPOOL::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_BRAINPOOL==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void BRAINPOOL::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void BRAINPOOL::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_BRAINPOOL==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_BRAINPOOL == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_BRAINPOOL > 2)
        FP_imul(&t,&t,QNRI_BRAINPOOL); // precomputed QNR
...
void BRAINPOOL::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int BRAINPOOL::ECP_generator(ECP *G)
{
    BIG x, y;
