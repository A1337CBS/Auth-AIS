...
using namespace C41417;

/* test for P=O point-at-infinity */
int C41417::ECP_isinf(ECP *P)
{

#if CURVETYPE_C41417==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_C41417==WEIERSTRASS
...
static void ECP_cswap(C41417::ECP *P, C41417::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_C41417!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(C41417::ECP *P, C41417::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_C41417!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_C41417!=MONTGOMERY

#if CURVETYPE_C41417!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(C41417::ECP *P, C41417::ECP W[], sign32 b)
{
    C41417::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int C41417::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void C41417::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_C41417!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
int C41417::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int C41417::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int C41417::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int C41417::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int C41417::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
#if CURVETYPE_C41417==EDWARDS || CURVETYPE_C41417==WEIERSTRASS
...
/* SU=120 */
void C41417::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void C41417::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void C41417::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_C41417==MONTGOMERY
    BIG x;
    ECP_get(x, P);
    W->len = MODBYTES_B416_60; // + 1;
    //W->val[0] = 0x06;
    ...
    {
        W->val[0] = 0x04;
        W->len = 2 * MODBYTES_B416_60 + 1;
        BIG_toBytes(&(W->val[1]), x);
        BIG_toBytes(&(W->val[MODBYTES_B416_60 + 1]), y);
    }
#endif
}
...
int C41417::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_C41417==MONTGOMERY
    BIG x;
    ...
    if (typ == 0x04)
    {
        BIG_fromBytes(y, &(W->val[MODBYTES_B416_60 + 1]));
...
void C41417::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void C41417::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_C41417==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  C41417::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void C41417::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void C41417::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_C41417==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void C41417::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B416_60 * BASEBITS_B416_60 + 1) / 2];
    int i, a, b, s, ns, nb;
...
void C41417::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_C41417==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_C41417 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_C41417 > 2)
        FP_imul(&t,&t,QNRI_C41417); // precomputed QNR
...
void C41417::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int C41417::ECP_generator(ECP *G)
{
    BIG x, y;
