...
int HIFIVE::ECP_isinf(ECP *P)
{

#if CURVETYPE_HIFIVE==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_HIFIVE==WEIERSTRASS
...
static void ECP_cswap(HIFIVE::ECP *P, HIFIVE::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_HIFIVE!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(HIFIVE::ECP *P, HIFIVE::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_HIFIVE!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_HIFIVE!=MONTGOMERY

#if CURVETYPE_HIFIVE!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(HIFIVE::ECP *P, HIFIVE::ECP W[], sign32 b)
{
    HIFIVE::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int HIFIVE::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void HIFIVE::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_HIFIVE!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
int HIFIVE::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int HIFIVE::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int HIFIVE::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int HIFIVE::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int HIFIVE::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
#if CURVETYPE_HIFIVE==EDWARDS || CURVETYPE_HIFIVE==WEIERSTRASS
...
/* SU=120 */
void HIFIVE::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void HIFIVE::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void HIFIVE::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_HIFIVE==MONTGOMERY
    BIG x;
    ECP_get(x, P);
    W->len = MODBYTES_B336_60; // + 1;
    //W->val[0] = 0x06;
    ...
    {
        W->val[0] = 0x04;
        W->len = 2 * MODBYTES_B336_60 + 1;
        BIG_toBytes(&(W->val[1]), x);
        BIG_toBytes(&(W->val[MODBYTES_B336_60 + 1]), y);
    }
#endif
}
...
int HIFIVE::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_HIFIVE==MONTGOMERY
    BIG x;
    ...
    if (typ == 0x04)
    {
        BIG_fromBytes(y, &(W->val[MODBYTES_B336_60 + 1]));
...
void HIFIVE::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void HIFIVE::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_HIFIVE==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  HIFIVE::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void HIFIVE::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void HIFIVE::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_HIFIVE==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void HIFIVE::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B336_60 * BASEBITS_B336_60 + 1) / 2];
    int i, a, b, s, ns, nb;
...
void HIFIVE::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_HIFIVE==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_HIFIVE == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_HIFIVE > 2)
        FP_imul(&t,&t,QNRI_HIFIVE); // precomputed QNR
...
void HIFIVE::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int HIFIVE::ECP_generator(ECP *G)
{
    BIG x, y;
