...
int NUMS384E::ECP_KEY_PAIR_GENERATE(csprng *RNG, octet* S, octet *W)
{
    BIG r, gx, gy, s;
    ECP G;
    int res = 0;
    ...
    if (RNG != NULL)
    {
        BIG_randtrunc(s, r, 2 * CURVE_SECURITY_NUMS384E, RNG);
    }
    else
    {
...
/* Validate public key */
int NUMS384E::ECP_PUBLIC_KEY_VALIDATE(octet *W)
{
    BIG q, r, wx, k;
    ECP WP;
    int valid, nb;
...
int NUMS384E::ECP_SVDP_DH(octet *S, octet *WD, octet *Z)
{
    BIG r, s, wx;
    int valid;
    ECP W;
...
int NUMS384E::ECP_SP_DSA(int hlen, csprng *RNG, octet *K, octet *S, octet *F, octet *C, octet *D)
{
    char h[128];
    ...
    int blen = H.len;
    if (H.len > MODBYTES_B384_56) blen = MODBYTES_B384_56;
...
int NUMS384E::ECP_VP_DSA(int hlen, octet *W, octet *F, octet *C, octet *D)
{
    char h[128];
    octet H = {0, sizeof(h), h};
    ...
    OCT_shl(C, C->len - MODBYTES_B384_56);
    OCT_shl(D, D->len - MODBYTES_B384_56);

    BIG_fromBytes(c, C->val);
    BIG_fromBytes(d, D->val);

    int blen = H.len;
    if (blen > MODBYTES_B384_56) blen = MODBYTES_B384_56;
...
void NUMS384E::ECP_ECIES_ENCRYPT(int hlen, octet *P1, octet *P2, csprng *RNG, octet *W, octet *M, int tlen, octet *V, octet *C, octet *T)
{

    int i, len;
    char z[EFS_NUMS384E], vz[3 * EFS_NUMS384E + 1], k[2 * AESKEY_NUMS384E], k1[AESKEY_NUMS384E], k2[AESKEY_NUMS384E], l2[8], u[EFS_NUMS384E];
    ...
    KDF2(MC_SHA2, hlen, &K, 2 * AESKEY_NUMS384E, &VZ, P1);

    K1.len = K2.len = AESKEY_NUMS384E;
    for (i = 0; i < AESKEY_NUMS384E; i++)
    {
        K1.val[i] = K.val[i];
        K2.val[i] = K.val[AESKEY_NUMS384E + i];
    }
...
int NUMS384E::ECP_ECIES_DECRYPT(int hlen, octet *P1, octet *P2, octet *V, octet *C, octet *T, octet *U, octet *M)
{

    int i, len;
    char z[EFS_NUMS384E], vz[3 * EFS_NUMS384E + 1], k[2 * AESKEY_NUMS384E], k1[AESKEY_NUMS384E], k2[AESKEY_NUMS384E], l2[8], tag[32];
    octet Z = {0, sizeof(z), z};
    octet VZ = {0, sizeof(vz), vz};
    octet K = {0, sizeof(k), k};
    octet K1 = {0, sizeof(k1), k1};
