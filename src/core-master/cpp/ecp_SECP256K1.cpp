...
static void ECP_cswap(SECP256K1::ECP *P, SECP256K1::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_SECP256K1!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(SECP256K1::ECP *P, SECP256K1::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_SECP256K1!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
static void ECP_select(SECP256K1::ECP *P, SECP256K1::ECP W[], sign32 b)
{
    SECP256K1::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int SECP256K1::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void SECP256K1::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_SECP256K1!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=56 */
void SECP256K1::ECP_rhs(FP *v, FP *x)
{
#if CURVETYPE_SECP256K1==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
...
int SECP256K1::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int SECP256K1::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int SECP256K1::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int SECP256K1::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int SECP256K1::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
/* SU=160 */
void SECP256K1::ECP_affine(ECP *P)
{
    FP one, iz;
...
/* SU=120 */
void SECP256K1::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void SECP256K1::ECP_output(ECP *P)
{
    BIG x, y;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void SECP256K1::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void SECP256K1::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_SECP256K1==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int SECP256K1::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_SECP256K1==MONTGOMERY
    BIG x;
...
void SECP256K1::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void SECP256K1::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_SECP256K1==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  SECP256K1::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void SECP256K1::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void SECP256K1::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_SECP256K1==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void SECP256K1::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void SECP256K1::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_SECP256K1==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_SECP256K1 == 2)
         FP_add(&t,&t,&t);  // 2t^2
    if (PM1D2_SECP256K1 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_SECP256K1 > 2)
        FP_imul(&t,&t,QNRI_SECP256K1); // precomputed QNR
...
void SECP256K1::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int SECP256K1::ECP_generator(ECP *G)
{
    BIG x, y;
