...
#include <stdlib.h>
#include <string.h>
#include "hpke_C41417.h"

void C41417::HPKE_Encap(int config_id,csprng *RNG,octet *SKE,octet *Z,octet *pkE,octet *pkR)
{
    int res,kem;
    char ske[EGS_C41417];
    ...
    if (RNG==NULL)
    {
        ...
        if (kem==2 || kem==4) {
            ...
            if (kem==2)
            {
                skE.val[EGS_C41417-1]&=248;  // x2EGS_C41417-119 scalar processing
                skE.val[0]&=127;
                skE.val[0]|=64;
            } else {
                skE.val[EGS_C41417-1]&=252;
                skE.val[0]|=128;
            }
        }
    }
...
void C41417::HPKE_Decap(int config_id,octet *Z,octet *pkE,octet *SKR)
{
    int res,kem;
    char skr[EGS_C41417];
    ...
    if (kem==2 || kem==4) {
        ...
        if (kem==2)
        {
            skR.val[EGS_C41417-1]&=248;
            skR.val[0]&=127;
            skR.val[0]|=64;
        } else {
            skR.val[EGS_C41417-1]&=252;
            skR.val[0]|=128;
        }       
    }
...
void C41417::HPKE_AuthEncap(int config_id,csprng *RNG,octet *SKE,octet *Z,octet *pkE,octet *pkR,octet *SKI)
{
    int res,kem;
    char ske[EGS_C41417];
    octet skE={0,sizeof(ske),ske};
    ...
    if (RNG==NULL)
    {
        ...
        if (kem==2 || kem==4) {
            ...
            if (kem==2)
            {
                skE.val[EGS_C41417-1]&=248;
                skE.val[0]&=127;
                skE.val[0]|=64;
                skI.val[EGS_C41417-1]&=248;
                skI.val[0]&=127;
                skI.val[0]|=64;
            } else {
                skE.val[EGS_C41417-1]&=252;
                skE.val[0]|=128;
                skI.val[EGS_C41417-1]&=252;
                skI.val[0]|=128;
            }
        }
    }
...
void C41417::HPKE_AuthDecap(int config_id,octet *Z,octet *pkE,octet *SKR,octet *pkI)
{
    int res,kem;
    char skr[EGS_C41417];
    octet skR={0,sizeof(skr),skr};
...
void C41417::HPKE_KeySchedule(int config_id,octet *key,octet *nonce,int mode,octet *pkR,octet *Z,octet *pkE,octet *info,octet *psk,octet *pskID,octet *pkI)
{
    char context[550];
    ...
    SPhash(MC_SHA2,HASH_TYPE_C41417,&H,pskID);
    OCT_joctet(&CONTEXT,&H);
    SPhash(MC_SHA2,HASH_TYPE_C41417,&H,info);
    OCT_joctet(&CONTEXT,&H);

    //printf("Context= "); OCT_output(&CONTEXT);
    HKDF_Extract(MC_SHA2,HASH_TYPE_C41417,&SECRET,psk,Z);
    //printf("Secret= "); OCT_output(&SECRET);

    OCT_jbytes(&FULL_CONTEXT,(char *)"hpke key",8); OCT_joctet(&FULL_CONTEXT,&CONTEXT);
    HKDF_Expand(MC_SHA2,HASH_TYPE_C41417,key,AESKEY_C41417,&SECRET,&FULL_CONTEXT);
    OCT_clear(&FULL_CONTEXT);
    OCT_jbytes(&FULL_CONTEXT,(char *)"hpke nonce",10); OCT_joctet(&FULL_CONTEXT,&CONTEXT);
    HKDF_Expand(MC_SHA2,HASH_TYPE_C41417,nonce,12,&SECRET,&FULL_CONTEXT);
}
