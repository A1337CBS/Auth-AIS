...
/* So when multiplying two numbers, their product *must* be less than MODBITS_SM2+BASEBITS_B256_56*NLEN_B256_56 */
/* Results *may* be one bit bigger than MODBITS_SM2 */

#if MODTYPE_SM2 == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void SM2::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void SM2::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void SM2::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_SM2);
    ...
    if (MConst < NEXCESS_B256_56)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B256_56 - 1];
        r[NLEN_B256_56 - 1] &= TMASK_SM2;
        r[0] += MConst * ((tw >> TBITS_SM2));
    }
    else
    {
        ...
        r[NLEN_B256_56 - 1] &= TMASK_SM2;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_SM2) + (v << (BASEBITS_B256_56 - TBITS_SM2))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_SM2) + (v << (BASEBITS_B256_56 - TBITS_SM2)));
#endif
    }
    BIG_norm(r);
}
...
#if MODTYPE_SM2 == GENERALISED_MERSENNE
...
void SM2::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_SM2);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_SM2 / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_SM2 / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B256_56 - 1] >> TBITS_SM2;
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B256_56 - 1] >> TBITS_SM2; // + (carry<<(BASEBITS_B256_56-TBITS_SM2));
    r[NLEN_B256_56 - 1] &= TMASK_SM2;
...
#if MODTYPE_SM2 == MONTGOMERY_FRIENDLY
...
void SM2::FP_from_int(FP *x,int a)
{
    BIG w;
...
void SM2::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void SM2::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void SM2::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void SM2::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void SM2::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int SM2::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
/* SU= 88 */
void SM2::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;

    if ((sign64)a->XES * b->XES > (sign64)FEXCESS_SM2)
    {
#ifdef DEBUG_REDUCE
...
/* SU= 136 */
void SM2::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
#if MODTYPE_SM2==PSEUDO_MERSENNE || MODTYPE_SM2==GENERALISED_MERSENNE
...
/* Set r=a^2 mod m */
/* SU= 88 */
void SM2::FP_sqr(FP *r, FP *a)
{
    DBIG d;

    if ((sign64)a->XES * a->XES > (sign64)FEXCESS_SM2)
    {
#ifdef DEBUG_REDUCE
        printf("Product too large - reducing it\n");
#endif
...
/* SU= 16 */
/* Set r=a+b */
void SM2::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
    if (r->XES > FEXCESS_SM2)
    {
#ifdef DEBUG_REDUCE
...
/* Set r=a-b mod m */
/* SU= 56 */
void SM2::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
void SM2::FP_pow(FP *r, FP *a, BIG b)
{
...
void SM2::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_SM2;
    FP xp[11], t, key;
...
// phase 1
    ...
    n = MODBITS_SM2;
#endif
#if MODTYPE_SM2==GENERALISED_MERSENNE  // Goldilocks ONLY
    n = MODBITS_SM2 / 2;
#endif
...
void SM2::FP_sqrt(FP *r, FP *a, FP* h)
{
    int i,j,k,u,e=PM1D2_SM2;
    FP v,g,t,b;
    BIG m;
