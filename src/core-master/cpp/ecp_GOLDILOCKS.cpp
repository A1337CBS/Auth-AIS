...
int GOLDILOCKS::ECP_isinf(ECP *P)
{
...
static void ECP_cswap(GOLDILOCKS::ECP *P, GOLDILOCKS::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_GOLDILOCKS!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(GOLDILOCKS::ECP *P, GOLDILOCKS::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_GOLDILOCKS!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_GOLDILOCKS!=MONTGOMERY

#if CURVETYPE_GOLDILOCKS!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(GOLDILOCKS::ECP *P, GOLDILOCKS::ECP W[], sign32 b)
{
    GOLDILOCKS::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int GOLDILOCKS::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void GOLDILOCKS::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_GOLDILOCKS!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=56 */
void GOLDILOCKS::ECP_rhs(FP *v, FP *x)
{
#if CURVETYPE_GOLDILOCKS==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
...
int GOLDILOCKS::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int GOLDILOCKS::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int GOLDILOCKS::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int GOLDILOCKS::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int GOLDILOCKS::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
/* SU=160 */
void GOLDILOCKS::ECP_affine(ECP *P)
{
    FP one, iz;
...
#if CURVETYPE_GOLDILOCKS==EDWARDS || CURVETYPE_GOLDILOCKS==WEIERSTRASS
...
/* SU=120 */
void GOLDILOCKS::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void GOLDILOCKS::ECP_output(ECP *P)
{
    BIG x, y;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void GOLDILOCKS::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void GOLDILOCKS::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_GOLDILOCKS==MONTGOMERY
    BIG x;
    ECP_get(x, P);
    W->len = MODBYTES_B448_58; // + 1;
    //W->val[0] = 0x06;
    ...
    {
        W->val[0] = 0x04;
        W->len = 2 * MODBYTES_B448_58 + 1;
        BIG_toBytes(&(W->val[1]), x);
        BIG_toBytes(&(W->val[MODBYTES_B448_58 + 1]), y);
    }
#endif
}
...
int GOLDILOCKS::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_GOLDILOCKS==MONTGOMERY
    BIG x;
    ...
    if (typ == 0x04)
    {
        BIG_fromBytes(y, &(W->val[MODBYTES_B448_58 + 1]));
...
void GOLDILOCKS::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void GOLDILOCKS::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_GOLDILOCKS==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  GOLDILOCKS::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void GOLDILOCKS::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void GOLDILOCKS::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_GOLDILOCKS==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void GOLDILOCKS::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B448_58 * BASEBITS_B448_58 + 1) / 2];
    int i, a, b, s, ns, nb;
...
void GOLDILOCKS::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_GOLDILOCKS==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_GOLDILOCKS == 2)
         FP_add(&t,&t,&t);  // 2t^2
    if (PM1D2_GOLDILOCKS == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_GOLDILOCKS > 2)
        FP_imul(&t,&t,QNRI_GOLDILOCKS); // precomputed QNR
...
void GOLDILOCKS::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int GOLDILOCKS::ECP_generator(ECP *G)
{
    BIG x, y;
