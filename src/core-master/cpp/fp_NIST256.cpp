...
/* So when multiplying two numbers, their product *must* be less than MODBITS_NIST256+BASEBITS_B256_56*NLEN_B256_56 */
/* Results *may* be one bit bigger than MODBITS_NIST256 */

#if MODTYPE_NIST256 == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void NIST256::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void NIST256::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void NIST256::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_NIST256);
    ...
    if (MConst < NEXCESS_B256_56)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B256_56 - 1];
        r[NLEN_B256_56 - 1] &= TMASK_NIST256;
        r[0] += MConst * ((tw >> TBITS_NIST256));
    }
    else
    {
        ...
        r[NLEN_B256_56 - 1] &= TMASK_NIST256;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_NIST256) + (v << (BASEBITS_B256_56 - TBITS_NIST256))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_NIST256) + (v << (BASEBITS_B256_56 - TBITS_NIST256)));
#endif
    }
    BIG_norm(r);
}
...
void NIST256::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_NIST256);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_NIST256 / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_NIST256 / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B256_56 - 1] >> TBITS_NIST256;
...
/*
    ...
    BIG_split(t2, b2, d, MBITS_NIST256);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B256_56 - 1] >> TBITS_NIST256; // + (carry<<(BASEBITS_B256_56-TBITS_NIST256));
    r[NLEN_B256_56 - 1] &= TMASK_NIST256;
...
void NIST256::FP_mod(BIG a, DBIG d)
{
    int i;
...
void NIST256::FP_from_int(FP *x,int a)
{
    BIG w;
...
void NIST256::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void NIST256::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void NIST256::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void NIST256::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void NIST256::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int NIST256::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
void NIST256::FP_rawoutput(FP *r)
{
    BIG_rawoutput(r->g);
}
...
/* SU= 88 */
void NIST256::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;

    if ((sign64)a->XES * b->XES > (sign64)FEXCESS_NIST256)
    {
#ifdef DEBUG_REDUCE
...
/* SU= 136 */
void NIST256::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
#if MODTYPE_NIST256==PSEUDO_MERSENNE || MODTYPE_NIST256==GENERALISED_MERSENNE
...
/* Set r=a^2 mod m */
/* SU= 88 */
void NIST256::FP_sqr(FP *r, FP *a)
{
    DBIG d;

    if ((sign64)a->XES * a->XES > (sign64)FEXCESS_NIST256)
    {
#ifdef DEBUG_REDUCE
        printf("Product too large - reducing it\n");
#endif
...
/* SU= 16 */
/* Set r=a+b */
void NIST256::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
    if (r->XES > FEXCESS_NIST256)
    {
#ifdef DEBUG_REDUCE
...
/* Set r=a-b mod m */
/* SU= 56 */
void NIST256::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
/* Set r=a/2. */
/* SU= 56 */
void NIST256::FP_div2(FP *r, FP *a)
{
    BIG m;
    BIG w;
    BIG_rcopy(m, Modulus);
...
void NIST256::FP_pow(FP *r, FP *a, BIG b)
{
...
void NIST256::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_NIST256;
    FP xp[11], t, key;
...
// phase 1
    ...
    n = MODBITS_NIST256;
#endif
#if MODTYPE_NIST256==GENERALISED_MERSENNE  // Goldilocks ONLY
    n = MODBITS_NIST256 / 2;
#endif
...
void NIST256::FP_invsqrt(FP *r,FP *x)
{
#if MODTYPE_NIST256==PSEUDO_MERSENNE  || MODTYPE_NIST256==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
int NIST256::FP_qr(FP *x,FP *h)
{
    FP r;
    int i,e=PM1D2_NIST256;
    FP_invsqrt(&r,x);
    if (h!=NULL)
        FP_copy(h,&r);
...
void NIST256::FP_sqrt(FP *r, FP *a, FP* h)
{
    int i,j,k,u,e=PM1D2_NIST256;
    FP v,g,t,b;
    BIG m;
