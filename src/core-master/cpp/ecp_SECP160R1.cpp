...
int SECP160R1::ECP_isinf(ECP *P)
{

#if CURVETYPE_SECP160R1==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_SECP160R1==WEIERSTRASS
...
static void ECP_cswap(SECP160R1::ECP *P, SECP160R1::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_SECP160R1!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(SECP160R1::ECP *P, SECP160R1::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_SECP160R1!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_SECP160R1!=MONTGOMERY

#if CURVETYPE_SECP160R1!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(SECP160R1::ECP *P, SECP160R1::ECP W[], sign32 b)
{
    SECP160R1::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int SECP160R1::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void SECP160R1::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_SECP160R1!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=56 */
void SECP160R1::ECP_rhs(FP *v, FP *x)
{
#if CURVETYPE_SECP160R1==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
...
int SECP160R1::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int SECP160R1::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int SECP160R1::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int SECP160R1::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int SECP160R1::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
/* SU=160 */
void SECP160R1::ECP_affine(ECP *P)
{
    FP one, iz;
...
#if CURVETYPE_SECP160R1==EDWARDS || CURVETYPE_SECP160R1==WEIERSTRASS
...
/* SU=120 */
void SECP160R1::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void SECP160R1::ECP_output(ECP *P)
{
    BIG x, y;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void SECP160R1::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void SECP160R1::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_SECP160R1==MONTGOMERY
    BIG x;
    ECP_get(x, P);
    W->len = MODBYTES_B160_56; // + 1;
    //W->val[0] = 0x06;
    ...
    {
        W->val[0] = 0x04;
        W->len = 2 * MODBYTES_B160_56 + 1;
        BIG_toBytes(&(W->val[1]), x);
        BIG_toBytes(&(W->val[MODBYTES_B160_56 + 1]), y);
    }
#endif
}
...
int SECP160R1::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_SECP160R1==MONTGOMERY
    BIG x;
    ...
    if (typ == 0x04)
    {
        BIG_fromBytes(y, &(W->val[MODBYTES_B160_56 + 1]));
...
void SECP160R1::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void SECP160R1::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_SECP160R1==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  SECP160R1::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void SECP160R1::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void SECP160R1::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_SECP160R1==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void SECP160R1::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
    sign8 w[1 + (NLEN_B160_56 * BASEBITS_B160_56 + 1) / 2];
    int i, a, b, s, ns, nb;
...
void SECP160R1::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_SECP160R1==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_SECP160R1 == 2)
         FP_add(&t,&t,&t);  // 2t^2
    if (PM1D2_SECP160R1 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_SECP160R1 > 2)
        FP_imul(&t,&t,QNRI_SECP160R1); // precomputed QNR
...
// Elligator 2 - map to Montgomery, place point, map back
    ...
    if (CURVE_A==1)
    {
        FP_add(&A,&B,&one);  // A=B+1
        FP_sub(&B,&B,&one);   // B=B-1
...
void SECP160R1::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int SECP160R1::ECP_generator(ECP *G)
{
    BIG x, y;
