...
int NIST384::ECP_isinf(ECP *P)
{

#if CURVETYPE_NIST384==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_NIST384==WEIERSTRASS
    return (FP_iszilch(&(P->x)) && FP_iszilch(&(P->z)));
#endif
#if CURVETYPE_NIST384==MONTGOMERY
...
static void ECP_cswap(NIST384::ECP *P, NIST384::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_NIST384!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(NIST384::ECP *P, NIST384::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_NIST384!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_NIST384!=MONTGOMERY

#if CURVETYPE_NIST384!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(NIST384::ECP *P, NIST384::ECP W[], sign32 b)
{
    NIST384::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int NIST384::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void NIST384::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_NIST384!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=56 */
void NIST384::ECP_rhs(FP *v, FP *x)
{
#if CURVETYPE_NIST384==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
...
int NIST384::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int NIST384::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int NIST384::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int NIST384::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int NIST384::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
#if CURVETYPE_NIST384==EDWARDS || CURVETYPE_NIST384==WEIERSTRASS
...
/* SU=120 */
void NIST384::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void NIST384::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void NIST384::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_NIST384==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int NIST384::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_NIST384==MONTGOMERY
    BIG x;
...
void NIST384::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void NIST384::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_NIST384==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  NIST384::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void NIST384::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void NIST384::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_NIST384==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void NIST384::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void NIST384::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_NIST384==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_NIST384 == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_NIST384 > 2)
        FP_imul(&t,&t,QNRI_NIST384); // precomputed QNR
...
void NIST384::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int NIST384::ECP_generator(ECP *G)
{
    BIG x, y;
