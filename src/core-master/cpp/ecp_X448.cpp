...
int X448::ECP_isinf(ECP *P)
{

#if CURVETYPE_X448==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_X448==WEIERSTRASS
...
static void ECP_cswap(X448::ECP *P, X448::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_X448!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(X448::ECP *P, X448::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_X448!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_X448!=MONTGOMERY

#if CURVETYPE_X448!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(X448::ECP *P, X448::ECP W[], sign32 b)
{
    X448::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int X448::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void X448::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_X448!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=16 */
int X448::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int X448::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int X448::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
#if CURVETYPE_X448==EDWARDS || CURVETYPE_X448==WEIERSTRASS
...
void X448::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_X448==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int X448::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_X448==MONTGOMERY
    BIG x;
...
void X448::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
void X448::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
void X448::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void X448::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_X448==MONTGOMERY
// Elligator 2
    BIG a;
...
void X448::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
