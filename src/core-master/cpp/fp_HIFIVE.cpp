...
/* So when multiplying two numbers, their product *must* be less than MODBITS_HIFIVE+BASEBITS_B336_60*NLEN_B336_60 */
/* Results *may* be one bit bigger than MODBITS_HIFIVE */

#if MODTYPE_HIFIVE == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void HIFIVE::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void HIFIVE::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void HIFIVE::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_HIFIVE);
    ...
    if (MConst < NEXCESS_B336_60)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B336_60 - 1];
        r[NLEN_B336_60 - 1] &= TMASK_HIFIVE;
        r[0] += MConst * ((tw >> TBITS_HIFIVE));
    }
    else
    {
        ...
        r[NLEN_B336_60 - 1] &= TMASK_HIFIVE;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_HIFIVE) + (v << (BASEBITS_B336_60 - TBITS_HIFIVE))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_HIFIVE) + (v << (BASEBITS_B336_60 - TBITS_HIFIVE)));
#endif
    }
    BIG_norm(r);
}
...
void HIFIVE::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_HIFIVE);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_HIFIVE / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_HIFIVE / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B336_60 - 1] >> TBITS_HIFIVE;

    r[NLEN_B336_60 - 1] &= TMASK_HIFIVE;
    r[0] += carry;

    r[224 / BASEBITS_B336_60] += carry << (224 % BASEBITS_B336_60); /* need to check that this falls mid-word */
    BIG_norm(r);

/*

    BIG t, b, t2, b2;
    int BTset = MBITS_HIFIVE / 2;
    chunk carry;
    BIG_split(t, b, d, MBITS_HIFIVE);
    ...
    BIG_split(t2, b2, d, MBITS_HIFIVE);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B336_60 - 1] >> TBITS_HIFIVE; // + (carry<<(BASEBITS_B336_60-TBITS_HIFIVE));
    r[NLEN_B336_60 - 1] &= TMASK_HIFIVE;

    r[BTset / BASEBITS_B336_60] += carry << (BTset % BASEBITS_B336_60); // need to check that this falls mid-word 
...
void HIFIVE::FP_mod(BIG a, DBIG d)
{
    int i;

    for (i = 0; i < NLEN_B336_60; i++)
        d[NLEN_B336_60 + i] += muladd(d[i], MConst - 1, d[i], &d[NLEN_B336_60 + i - 1]);
...
void HIFIVE::FP_from_int(FP *x,int a)
{
    BIG w;
...
void HIFIVE::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void HIFIVE::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void HIFIVE::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void HIFIVE::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void HIFIVE::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int HIFIVE::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
/* SU= 88 */
void HIFIVE::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;
...
/* SU= 136 */
void HIFIVE::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
/* SU= 16 */
/* Set r=a+b */
void HIFIVE::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
...
/* Set r=a-b mod m */
/* SU= 56 */
void HIFIVE::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
static int quo(BIG n, BIG m)
{
    ...
    {
        sh = hb - TBITS_HIFIVE;
        num = (n[NLEN_B336_60 - 1] << sh) | (n[NLEN_B336_60 - 2] >> (BASEBITS_B336_60 - sh));
        den = (m[NLEN_B336_60 - 1] << sh) | (m[NLEN_B336_60 - 2] >> (BASEBITS_B336_60 - sh));
    }
    else
    {
        num = n[NLEN_B336_60 - 1];
        den = m[NLEN_B336_60 - 1];
    }
...
void HIFIVE::FP_reduce(FP *a)
{
    ...
    if (a->XES > 16)
    {
        ...
        r[NLEN_B336_60 - 1] += (carry << BASEBITS_B336_60); // correction - put any carry out back in again
        BIG_sub(a->g, a->g, r);
        BIG_norm(a->g);
        sb = 2;
    }
...
void HIFIVE::FP_pow(FP *r, FP *a, BIG b)
{
    sign8 w[1 + (NLEN_B336_60 * BASEBITS_B336_60 + 3) / 4];
    FP tb[16];
    BIG t;
    int i, nb;
...
void HIFIVE::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_HIFIVE;
    FP xp[11], t, key;
...
void HIFIVE::FP_invsqrt(FP *r,FP *x)
{
#if MODTYPE_HIFIVE==PSEUDO_MERSENNE  || MODTYPE_HIFIVE==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
void HIFIVE::FP_sqrt(FP *r, FP *a, FP* h)
{
