...
/* So when multiplying two numbers, their product *must* be less than MODBITS_F256PME+BASEBITS_B256_56*NLEN_B256_56 */
/* Results *may* be one bit bigger than MODBITS_F256PME */

#if MODTYPE_F256PME == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void F256PME::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void F256PME::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void F256PME::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_F256PME);
    ...
    if (MConst < NEXCESS_B256_56)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B256_56 - 1];
        r[NLEN_B256_56 - 1] &= TMASK_F256PME;
        r[0] += MConst * ((tw >> TBITS_F256PME));
    }
    else
    {
        ...
        r[NLEN_B256_56 - 1] &= TMASK_F256PME;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_F256PME) + (v << (BASEBITS_B256_56 - TBITS_F256PME))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_F256PME) + (v << (BASEBITS_B256_56 - TBITS_F256PME)));
#endif
    }
    BIG_norm(r);
}
...
#if MODTYPE_F256PME == GENERALISED_MERSENNE
...
void F256PME::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_F256PME);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_F256PME / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_F256PME / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B256_56 - 1] >> TBITS_F256PME;
...
/*
    ...
    BIG_split(t2, b2, d, MBITS_F256PME);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B256_56 - 1] >> TBITS_F256PME; // + (carry<<(BASEBITS_B256_56-TBITS_F256PME));
    r[NLEN_B256_56 - 1] &= TMASK_F256PME;
...
#if MODTYPE_F256PME == MONTGOMERY_FRIENDLY
...
void F256PME::FP_mod(BIG a, DBIG d)
{
    int i;
...
#if MODTYPE_F256PME == NOT_SPECIAL
...
void F256PME::FP_from_int(FP *x,int a)
{
    BIG w;
...
void F256PME::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void F256PME::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void F256PME::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void F256PME::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void F256PME::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int F256PME::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
void F256PME::FP_rawoutput(FP *r)
{
    BIG_rawoutput(r->g);
}
...
/* SU= 88 */
void F256PME::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;

    if ((sign64)a->XES * b->XES > (sign64)FEXCESS_F256PME)
    {
#ifdef DEBUG_REDUCE
...
/* SU= 136 */
void F256PME::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
#if MODTYPE_F256PME==PSEUDO_MERSENNE || MODTYPE_F256PME==GENERALISED_MERSENNE
...
/* Set r=a^2 mod m */
/* SU= 88 */
void F256PME::FP_sqr(FP *r, FP *a)
{
    DBIG d;

    if ((sign64)a->XES * a->XES > (sign64)FEXCESS_F256PME)
    {
#ifdef DEBUG_REDUCE
        printf("Product too large - reducing it\n");
#endif
...
/* SU= 16 */
/* Set r=a+b */
void F256PME::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
    if (r->XES > FEXCESS_F256PME)
    {
#ifdef DEBUG_REDUCE
...
/* Set r=a-b mod m */
/* SU= 56 */
void F256PME::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
/* Set r=a/2. */
/* SU= 56 */
void F256PME::FP_div2(FP *r, FP *a)
{
    BIG m;
    BIG w;
    BIG_rcopy(m, Modulus);
...
void F256PME::FP_pow(FP *r, FP *a, BIG b)
{
...
void F256PME::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_F256PME;
    FP xp[11], t, key;
...
// phase 1
    ...
    n = MODBITS_F256PME;
#endif
#if MODTYPE_F256PME==GENERALISED_MERSENNE  // Goldilocks ONLY
    n = MODBITS_F256PME / 2;
#endif
...
void F256PME::FP_invsqrt(FP *r,FP *x)
{
#if MODTYPE_F256PME==PSEUDO_MERSENNE  || MODTYPE_F256PME==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
int F256PME::FP_qr(FP *x,FP *h)
{
    FP r;
    int i,e=PM1D2_F256PME;
    FP_invsqrt(&r,x);
    if (h!=NULL)
        FP_copy(h,&r);
...
void F256PME::FP_sqrt(FP *r, FP *a, FP* h)
{
    int i,j,k,u,e=PM1D2_F256PME;
    FP v,g,t,b;
    BIG m;
