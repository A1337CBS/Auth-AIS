...
using namespace F256PMW;

/* test for P=O point-at-infinity */
int NUMS256W::ECP_isinf(ECP *P)
{

#if CURVETYPE_NUMS256W==EDWARDS
    return (FP_iszilch(&(P->x)) && FP_equals(&(P->y), &(P->z)));
#endif
#if CURVETYPE_NUMS256W==WEIERSTRASS
...
static void ECP_cswap(NUMS256W::ECP *P, NUMS256W::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_NUMS256W!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(NUMS256W::ECP *P, NUMS256W::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_NUMS256W!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
#endif // CURVETYPE_NUMS256W!=MONTGOMERY

#if CURVETYPE_NUMS256W!=MONTGOMERY
/* Constant time select from pre-computed table */
static void ECP_select(NUMS256W::ECP *P, NUMS256W::ECP W[], sign32 b)
{
    NUMS256W::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int NUMS256W::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void NUMS256W::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_NUMS256W!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=56 */
void NUMS256W::ECP_rhs(FP *v, FP *x)
{
#if CURVETYPE_NUMS256W==WEIERSTRASS
    /* x^3+Ax+B */
    FP t;
...
int NUMS256W::ECP_set(ECP *P, BIG x)
{
    FP rhs;
    FP_nres(&rhs, x);
...
int NUMS256W::ECP_get(BIG x, ECP *P)
{
    ECP W;
    ECP_copy(&W, P);
    ECP_affine(&W);
...
/* SU=16 */
int NUMS256W::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int NUMS256W::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int NUMS256W::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
/* SU=160 */
void NUMS256W::ECP_affine(ECP *P)
{
    FP one, iz;
...
#if CURVETYPE_NUMS256W==EDWARDS || CURVETYPE_NUMS256W==WEIERSTRASS
...
/* SU=120 */
void NUMS256W::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void NUMS256W::ECP_output(ECP *P)
{
    BIG x, y;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void NUMS256W::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void NUMS256W::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_NUMS256W==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int NUMS256W::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_NUMS256W==MONTGOMERY
    BIG x;
...
void NUMS256W::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void NUMS256W::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_NUMS256W==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  NUMS256W::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void NUMS256W::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void NUMS256W::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_NUMS256W==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void NUMS256W::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void NUMS256W::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_NUMS256W==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_F256PMW == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_F256PMW > 2)
        FP_imul(&t,&t,QNRI_F256PMW); // precomputed QNR
...
void NUMS256W::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
...
int NUMS256W::ECP_generator(ECP *G)
{
    BIG x, y;
