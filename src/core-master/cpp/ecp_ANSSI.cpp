...
using namespace ANSSI;

/* test for P=O point-at-infinity */
int ANSSI::ECP_isinf(ECP *P)
{
...
static void ECP_cswap(ANSSI::ECP *P, ANSSI::ECP *Q, int d)
{
    FP_cswap(&(P->x), &(Q->x), d);
#if CURVETYPE_ANSSI!=MONTGOMERY
    FP_cswap(&(P->y), &(Q->y), d);
...
static void ECP_cmove(ANSSI::ECP *P, ANSSI::ECP *Q, int d)
{
    FP_cmove(&(P->x), &(Q->x), d);
#if CURVETYPE_ANSSI!=MONTGOMERY
    FP_cmove(&(P->y), &(Q->y), d);
...
static void ECP_select(ANSSI::ECP *P, ANSSI::ECP W[], sign32 b)
{
    ANSSI::ECP MP;
    sign32 m = b >> 31;
    sign32 babs = (b ^ m) - m;
...
/* Test P == Q */
/* SU=168 */
int ANSSI::ECP_equals(ECP *P, ECP *Q)
{
    FP a, b;
...
/* Set P=Q */
/* SU=16 */
void ANSSI::ECP_copy(ECP *P, ECP *Q)
{
    FP_copy(&(P->x), &(Q->x));
#if CURVETYPE_ANSSI!=MONTGOMERY
    FP_copy(&(P->y), &(Q->y));
...
/* SU=16 */
int ANSSI::ECP_get(BIG x, BIG y, ECP *P)
{
    int s;
    ECP W;
    ECP_copy(&W, P);
...
/* Set P=(x,{y}) */
/* SU=96 */
int ANSSI::ECP_set(ECP *P, BIG x, BIG y)
{
    FP rhs, y2;
...
/* SU=136 */
int ANSSI::ECP_setx(ECP *P, BIG x, int s)
{
    FP rhs,hint;
    BIG t;
...
/* SU=120 */
void ANSSI::ECP_outputxyz(ECP *P)
{
    BIG x, z;
    if (ECP_isinf(P))
    {
...
/* SU=16 */
/* Output point P */
void ANSSI::ECP_rawoutput(ECP *P)
{
    BIG x, y, z;
...
void ANSSI::ECP_toOctet(octet *W, ECP *P, bool compress)
{
#if CURVETYPE_ANSSI==MONTGOMERY
    BIG x;
    ECP_get(x, P);
...
int ANSSI::ECP_fromOctet(ECP *P, octet *W)
{
#if CURVETYPE_ANSSI==MONTGOMERY
    BIG x;
...
void ANSSI::ECP_add(ECP *P, ECP *Q, ECP *W)
{
    FP A, B, C, D, DA, CB;
...
/* Set P+=Q */
/* SU=248 */
void ANSSI::ECP_add(ECP *P, ECP *Q)
{
#if CURVETYPE_ANSSI==WEIERSTRASS
...
/* Set P-=Q */
/* SU=16 */
void  ANSSI::ECP_sub(ECP *P, ECP *Q)
{
    ECP NQ;
    ECP_copy(&NQ, Q);
    ECP_neg(&NQ);
...
void ANSSI::ECP_pinmul(ECP *P, int e, int bts)
{
    int i, b;
    ECP R0, R1;
...
/* Set P=r*P */
/* SU=424 */
void ANSSI::ECP_mul(ECP *P, BIG e)
{
#if CURVETYPE_ANSSI==MONTGOMERY
    /* Montgomery ladder */
    int nb, i, b;
...
void ANSSI::ECP_mul2(ECP *P, ECP *Q, BIG e, BIG f)
{
    BIG te, tf, mt;
    ECP S, T, W[8], C;
...
void ANSSI::ECP_hashit(ECP *P,BIG h)
{
#if CURVETYPE_ANSSI==MONTGOMERY
// Elligator 2
    BIG a;
    ...
    if (PM1D2_ANSSI == 1)
        FP_neg(&t,&t);      // -t^2
    if (PM1D2_ANSSI > 2)
        FP_imul(&t,&t,QNRI_ANSSI); // precomputed QNR
...
void ANSSI::ECP_mapit(ECP *P, octet *W)
{
    BIG q, x;
    DBIG dx;
    BIG_rcopy(q, Modulus);
