...
/* So when multiplying two numbers, their product *must* be less than MODBITS_C1665+BASEBITS_B168_60*NLEN_B168_60 */
/* Results *may* be one bit bigger than MODBITS_C1665 */

#if MODTYPE_C1665 == PSEUDO_MERSENNE
/* r=d mod m */

/* Converts from BIG integer to residue form mod Modulus */
void C1665::FP_nres(FP *y, BIG x)
{
    BIG_copy(y->g, x);
    y->XES = 1;
}
...
void C1665::FP_redc(BIG x, FP *y)
{
    BIG_copy(x, y->g);
}

/* reduce a DBIG to a BIG exploiting the special form of the modulus */
void C1665::FP_mod(BIG r, DBIG d)
{
    BIG t, b;
    chunk v, tw;
    BIG_split(t, b, d, MODBITS_C1665);
    ...
    if (MConst < NEXCESS_B168_60)
    {
        ...
        BIG_norm(r);
        tw = r[NLEN_B168_60 - 1];
        r[NLEN_B168_60 - 1] &= TMASK_C1665;
        r[0] += MConst * ((tw >> TBITS_C1665));
    }
    else
    {
        ...
        r[NLEN_B168_60 - 1] &= TMASK_C1665;
#if CHUNK == 16
        r[1] += muladd(MConst, ((tw >> TBITS_C1665) + (v << (BASEBITS_B168_60 - TBITS_C1665))), 0, &r[0]);
#else
        r[0] += MConst * ((tw >> TBITS_C1665) + (v << (BASEBITS_B168_60 - TBITS_C1665)));
#endif
    }
    BIG_norm(r);
}
...
#if MODTYPE_C1665 == GENERALISED_MERSENNE
...
void C1665::FP_mod(BIG r, DBIG d)
{
    ...
    BIG t, b;
    chunk carry;
    BIG_split(t, b, d, MBITS_C1665);

    BIG_add(r, t, b);

    BIG_dscopy(d, t);
    BIG_dshl(d, MBITS_C1665 / 2);
    ...
    BIG_add(r, r, b);
    BIG_norm(r);
    BIG_shl(t, MBITS_C1665 / 2);

    BIG_add(r, r, t);

    carry = r[NLEN_B168_60 - 1] >> TBITS_C1665;

    r[NLEN_B168_60 - 1] &= TMASK_C1665;
    r[0] += carry;

    r[224 / BASEBITS_B168_60] += carry << (224 % BASEBITS_B168_60); /* need to check that this falls mid-word */
    BIG_norm(r);

/*

    BIG t, b, t2, b2;
    int BTset = MBITS_C1665 / 2;
    chunk carry;
    BIG_split(t, b, d, MBITS_C1665);
...
//      carry=BIG_pmul(t,t,MConst);
    ...
    carry = r[NLEN_B168_60 - 1] >> TBITS_C1665; // + (carry<<(BASEBITS_B168_60-TBITS_C1665));
    r[NLEN_B168_60 - 1] &= TMASK_C1665;

    r[BTset / BASEBITS_B168_60] += carry << (BTset % BASEBITS_B168_60); // need to check that this falls mid-word 
...
#if MODTYPE_C1665 == MONTGOMERY_FRIENDLY
...
void C1665::FP_mod(BIG a, DBIG d)
{
    int i;

    for (i = 0; i < NLEN_B168_60; i++)
        d[NLEN_B168_60 + i] += muladd(d[i], MConst - 1, d[i], &d[NLEN_B168_60 + i - 1]);
...
void C1665::FP_from_int(FP *x,int a)
{
    BIG w;
...
void C1665::FP_copy(FP *y, FP *x)
{
    BIG_copy(y->g, x->g);
    y->XES = x->XES;
}

void C1665::FP_rcopy(FP *y, const BIG c)
{
    BIG b;
    BIG_rcopy(b, c);
    FP_nres(y, b);
...
/* Swap a and b if d=1 */
void C1665::FP_cswap(FP *a, FP *b, int d)
{
    sign32 t, c = d;
    BIG_cswap(a->g, b->g, d);
...
/* Move b to a if d=1 */
void C1665::FP_cmove(FP *a, FP *b, int d)
{
    sign32 c = -d;
...
void C1665::FP_zero(FP *x)
{
    BIG_zero(x->g);
    x->XES = 1;
}

int C1665::FP_equals(FP *x, FP *y)
{
    FP xg, yg;
    FP_copy(&xg, x);
    FP_copy(&yg, y);
...
/* SU= 88 */
void C1665::FP_mul(FP *r, FP *a, FP *b)
{
    DBIG d;

    if ((sign64)a->XES * b->XES > (sign64)FEXCESS_C1665)
    {
#ifdef DEBUG_REDUCE
...
/* SU= 136 */
void C1665::FP_imul(FP *r, FP *a, int c)
{
    DBIG d;
    BIG k;
    FP f;
...
#if MODTYPE_C1665==PSEUDO_MERSENNE || MODTYPE_C1665==GENERALISED_MERSENNE
...
/* Set r=a^2 mod m */
/* SU= 88 */
void C1665::FP_sqr(FP *r, FP *a)
{
    DBIG d;

    if ((sign64)a->XES * a->XES > (sign64)FEXCESS_C1665)
    {
#ifdef DEBUG_REDUCE
        printf("Product too large - reducing it\n");
#endif
...
/* SU= 16 */
/* Set r=a+b */
void C1665::FP_add(FP *r, FP *a, FP *b)
{
    BIG_add(r->g, a->g, b->g);
    r->XES = a->XES + b->XES;
    if (r->XES > FEXCESS_C1665)
    {
#ifdef DEBUG_REDUCE
...
/* Set r=a-b mod m */
/* SU= 56 */
void C1665::FP_sub(FP *r, FP *a, FP *b)
{
    FP n;
    FP_neg(&n, b);
    FP_add(r, a, &n);
...
static int quo(BIG n, BIG m)
{
    ...
    {
        sh = hb - TBITS_C1665;
        num = (n[NLEN_B168_60 - 1] << sh) | (n[NLEN_B168_60 - 2] >> (BASEBITS_B168_60 - sh));
        den = (m[NLEN_B168_60 - 1] << sh) | (m[NLEN_B168_60 - 2] >> (BASEBITS_B168_60 - sh));
    }
    else
    {
        num = n[NLEN_B168_60 - 1];
        den = m[NLEN_B168_60 - 1];
    }
...
void C1665::FP_reduce(FP *a)
{
    ...
    if (a->XES > 16)
    {
        ...
        r[NLEN_B168_60 - 1] += (carry << BASEBITS_B168_60); // correction - put any carry out back in again
        BIG_sub(a->g, a->g, r);
        BIG_norm(a->g);
        sb = 2;
    }
...
void C1665::FP_pow(FP *r, FP *a, BIG b)
{
    sign8 w[1 + (NLEN_B168_60 * BASEBITS_B168_60 + 3) / 4];
    FP tb[16];
    BIG t;
    int i, nb;
...
void C1665::FP_fpow(FP *r, FP *x)
{
    int i, j, k, bw, w, nw, lo, m, n, c, nd, e=PM1D2_C1665;
    FP xp[11], t, key;
...
// phase 1
    ...
    n = MODBITS_C1665;
#endif
#if MODTYPE_C1665==GENERALISED_MERSENNE  // Goldilocks ONLY
    n = MODBITS_C1665 / 2;
#endif
...
void C1665::FP_invsqrt(FP *r,FP *x)
{
#if MODTYPE_C1665==PSEUDO_MERSENNE  || MODTYPE_C1665==GENERALISED_MERSENNE
    FP_fpow(r, x);  
#else
...
void C1665::FP_sqrt(FP *r, FP *a, FP* h)
{
    int i,j,k,u,e=PM1D2_C1665;
    FP v,g,t,b;
    BIG m;
